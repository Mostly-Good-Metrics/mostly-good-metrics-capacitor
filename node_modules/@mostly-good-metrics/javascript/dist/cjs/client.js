...
class MostlyGoodMetrics {
    ...
    constructor(config) {
        ...
        storage_1.persistence.configureCookies(config.cookieDomain, config.disableCookies);
        this.anonymousIdValue = storage_1.persistence.initializeAnonymousId(config.anonymousId, utils_1.generateAnonymousId);
        // Set up logging
     ...
     */
    static identify(userId, profile) {
    ...
    track(name, properties) {
        ...
        const superProperties = storage_1.persistence.getSuperProperties();
        ...
        const mergedProperties = {
            ...
            [types_1.SystemProperties.SDK]: this.config.sdk,
        };
        const event = {
            name,
            client_event_id: (0, utils_1.generateUUID)(),
            ...
            os_version: this.config.osVersion || (0, utils_1.getOSVersion)() || undefined,
            environment: this.config.environment,
            locale: (0, utils_1.getLocale)(),
            timezone: (0, utils_1.getTimezone)(),
    ...
    sendIdentifyEventIfNeeded(userId, profile) {
        ...
        const storedHash = storage_1.persistence.getIdentifyHash();
        const lastSentAt = storage_1.persistence.getIdentifyLastSentAt();
        const now = Date.now();
        ...
        if (hashChanged || expiredTime) {
            logger_1.logger.debug(`Sending $identify event (hashChanged=${hashChanged}, expiredTime=${expiredTime})`);
            ...
            this.track(types_1.SystemEvents.IDENTIFY, properties);
            // Update stored hash and timestamp
            storage_1.persistence.setIdentifyHash(currentHash);
            storage_1.persistence.setIdentifyLastSentAt(now);
        }
        else {
            logger_1.logger.debug('Skipping $identify event (debounced)');
        }
    }
    /**
    ...
    resetIdentity() {
        ...
        storage_1.persistence.clearIdentifyState();
    }
    /**
     * Start a new session.
     */
    ...
    setSuperProperty(key, value) {
        logger_1.logger.debug(`Setting super property: ${key}`);
        storage_1.persistence.setSuperProperty(key, value);
    }
    /**
     * Set multiple super properties at once.
     */
    setSuperProperties(properties) {
        logger_1.logger.debug(`Setting super properties: ${Object.keys(properties).join(', ')}`);
        storage_1.persistence.setSuperProperties(properties);
    }
    /**
     * Remove a single super property.
     */
    removeSuperProperty(key) {
        logger_1.logger.debug(`Removing super property: ${key}`);
        storage_1.persistence.removeSuperProperty(key);
    }
    /**
     * Clear all super properties.
     */
    clearSuperProperties() {
        logger_1.logger.debug('Clearing all super properties');
        storage_1.persistence.clearSuperProperties();
    }
    /**
     * Get all current super properties.
     */
    getSuperProperties() {
        return storage_1.persistence.getSuperProperties();
    }
    /**
            ...
            if (result.success) {
                ...
                if (this.config.onError) {
                    try {
                        ...
                        logger_1.logger.error('Error in onError callback', e);
                    }
                }
